...
=head1 SYNOPSIS
    ...
    my $storage = Net::BitTorrent::Storage->new(
        base_path  => "./downloads",
        file_tree  => $bencoded_tree, # From BEP 52
        piece_size => 262144
    );
...
=item * B<BitTorrent v2 (BEP 52)>: Uses C<pieces root> (Merkle roots) to identify and verify files. Each file is verified independently.
...
Verifies a 16KiB block against the Merkle tree for the file identified by C<$root>. Returns boolean. Part of B<BEP 52>.
...
padding files/space defined in B<BEP 52>.

=head2 C<map_v2_piece( $index )>

Returns the C<pieces_root> and relative index for a v2 piece. Since v2 pieces are aligned to files, a piece always
belongs to exactly one file (or padding).
