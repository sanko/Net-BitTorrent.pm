...
C<Net::BitTorrent::Protocol::MSE> implements the Message Stream Encryption (MSE) protocol, also known as Protocol
Encryption (PE). This protocol provides a layer of obfuscation for BitTorrent traffic, helping to prevent passive
traffic analysis and "throttling" by ISPs.
...
Feeds raw bytes from the transport layer. During the handshake, it advances the MSE state machine. Once the state is
C<PAYLOAD>, it returns the decrypted plaintext.
...
=item * C<A_WAIT_SELECT>, C<B_WAIT_REQS>: Synchronizing on the info-hash.
