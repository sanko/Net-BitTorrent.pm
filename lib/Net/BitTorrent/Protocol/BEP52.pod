...
It specifically handles the synchronization of Merkle tree hashes  between peers, which is required for block-level
data verification.
...
Sends a C<HASH_REQUEST> (ID 21) message.

Parameters:

=over

=item * C<$pieces_root>: The 32-byte binary SHA-256 Merkle root of the file.

=item * C<$proof_layer>: The height of the layer containing the proof nodes.

=item * C<$base_layer>: The height of the layer being requested.

=item * C<$index>: The start index within the base layer.

=item * C<$length>: The number of hashes requested.
