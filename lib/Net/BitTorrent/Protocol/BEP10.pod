...
Net::BitTorrent::Protocol::BEP10 - Extension Protocol (v2.0.0)
...
C<Net::BitTorrent::Protocol::BEP10> implements the B<Extension Protocol (BEP 10)>. This is the  standard mechanism
modern BitTorrent clients use to negotiate support for additional  features (like PEX, Magnet links, and DHT security)
without needing to allocate new  reserved bits in the foundational BitTorrent handshake.
...
Sends a message for the extension identified by C<$name>. The module automatically looks up   the remote peer's
assigned ID for that extension.
