...
C<start( )> triggers DHT lookups, tracker announces, and local peer discovery.
...
Currently emits the following events: C<piece_verified>, C<piece_failed>, C<status_update>, C<peer_discovered>,
C<started>, C<stopped>, C<paused>, C<resumed>.
...
The name from the metadata if returned or a hex info-hash as a fallback.
...
Returns an arrayref of absolute paths (as L<Path::Tiny> objects) for all files in the torrent.
