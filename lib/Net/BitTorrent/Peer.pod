...
C<Net::BitTorrent::Peer> represents a single connection to a remote BitTorrent client. It maintains the choking/interested state machine and handles all message routing for a specific neighbor.
...
=itme Peers start with a reputation of 100.

=item Providing a verified piece increases reputation by 1.

=itme Providing a corrupt piece (failed hash) decreases reputation by 20.
...
Sends a C<CHOKE> or C<UNCHOKE> message to the peer. This controls whether the remote peer is allowed to request blocks from us.
