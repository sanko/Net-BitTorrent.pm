...
Net::BitTorrent::Tracker::HTTP - HTTP/HTTPS Tracker Protocol (v2.0.0)
...
URL-encoding of binary info-hashes and parsing of  bencoded responses.

=head1 METHODS

=head2 C<perform_announce( \%params, [$callback] )>

Executes an C<HTTP GET> request to the tracker.  * Automatically handles compact peer lists (IPv4 and IPv6). * Supports
B<Asynchronous Mode>: if C<$params-E<gt>{ua}> is provided (e.g., a    L<Mojo::UserAgent> instance), the request will be
non-blocking.
