...
Net::BitTorrent::Tracker::WebSeed - HTTP/FTP Seeding (BEP 19) (v2.1.0)
...
Attempts to fetch a piece from the web seed.  * C<$segments>: An arrayref of segment hashrefs (mapped by
L<Net::BitTorrent::Storage>). * Uses B<HTTP Range Requests> to fetch only the specific bytes required for the piece. *
Automatically follows up to 5 redirects. * Handles B<410 Gone> status codes by permanently disabling the web seed
instance.
