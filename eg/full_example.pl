...
# This example uses a common Linux ISO magnet link (Debian 13)
...
$torrent->on(
    status_update => sub ( $t, $stats ) {
        ...
        if ( !$metadata_done && $t->is_metadata_complete ) {
            say 'Metadata received! Starting download of: ' . $t->name;
            say 'Files:';
            say "  - $_" for $t->files->@*;
            $metadata_done = 1;
        }
...
$torrent->on(
    piece_verified => sub ( $t, $index ) {
        printf '%sProgress: [%-20s] %.2f%% (%d peers)', "\r", '#' x int( $t->progress / 5 ), $t->progress, scalar( $t->discovered_peers->@* );
    }
);
