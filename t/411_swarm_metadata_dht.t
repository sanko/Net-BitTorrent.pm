...
subtest 'Metadata Exchange' => sub {
    ...
    my $t_l      = $client_l->add_magnet( 'magnet:?xt=urn:btih:' . unpack( 'H*', $ih ), $leecher_dir );
    $t_l->start();
    is $t_l->state, 'METADATA', 'Leecher starts in METADATA state';
    ...
    is $t_l->state, 'RUNNING', 'Leecher state is now RUNNING';
...
subtest 'DHT Peer Discovery' => sub {
    ...
    my $ih         = 'A' x 20;
    my $t          = $client->add_magnet( 'magnet:?xt=urn:btih:' . unpack( 'H*', $ih ), '.' );
    my $discovered = 0;
    $t->on( 'peer_discovered', sub { $discovered++ } );
...
subtest 'Bridge Node (Metadata only)' => sub {
    ...
    my $t_b      = $client_b->add_magnet( 'magnet:?xt=urn:btih:' . unpack( 'H*', $ih ), $temp->child('bridge') );
    $t_b->handle_metadata_data( undef, 0, length( bencode($info) ), bencode($info) );
    $t_b->start();
    my $client_c = Net::BitTorrent->new( bep05 => 0, upnp_enabled => 0 );
    my $t_c      = $client_c->add_magnet( 'magnet:?xt=urn:btih:' . unpack( 'H*', $ih ), $temp->child('leecher') );
    $t_c->start();
    ...
    my $peer_b
        = Net::BitTorrent::Peer->new( protocol => $p_b, torrent => $t_b, transport => $trans_b, ip => '1.2.3.4', port => 80, encryption => 'none' );
    my $peer_c
        = Net::BitTorrent::Peer->new( protocol => $p_c, torrent => $t_c, transport => $trans_c, ip => '4.3.2.1', port => 80, encryption => 'none' );
    $t_b->register_peer_object($peer_b);
    $t_c->register_peer_object($peer_c);
    $trans_b->emit('connected');
    $trans_c->emit('connected');
    my $max_iter = 100;
